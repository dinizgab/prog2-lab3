...
public class MainAgenda {
  ...
  private static void cadastraContato(Agenda agenda, Scanner scanner) {
    System.out.print("\nPosição na agenda> ");
    String entradaPosicao = scanner.next();
    int posicao = Integer.parseInt(entradaPosicao);

    if (!Contato.validaPosicao(posicao)) {
    ...
    if (!Contato.validaCampoVazio(nome)) {
    ...
    if (!Contato.validaRepeticao(nome, sobrenome, agenda)) {
    ...
    if (!Contato.validaCampoVazio(telefone)) {
      System.out.println("CONTATO INVALIDO");
      return;
    }
  ...
  private static void listaFavoritos(Agenda agenda) {
    ...
    for (int i = 0; i < listaDeFavoritos.length; i++) {
      if (listaDeFavoritos[i] != null) {
        System.out.println(formataContato(i, listaDeFavoritos[i]));
      }
    }
  }
  ...
  private static void adicionaFavoritos(Agenda agenda, Scanner scanner) {
    ...
    if (!agenda.validaFavorito(posicaoContato)) {
  ...
  private static void adicionaTags(Agenda agenda, Scanner scanner) {
    ...
    agenda.cadastraTag(contatos, tag, posicaoTag);
  }
